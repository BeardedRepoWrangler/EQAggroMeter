...
**Empty set: don't broadcast.** When my recent-attacker set drops to empty (no mobs hitting me anymore), I stop publishing AGMH lines. Receivers age out my entry via the remote TTL. Trade-off: there's a brief window (up to remote TTL) where peers think I'm still being hit when I'm not. Accepted because: (a) the priority chain falls through to other priorities when no signal is available, so attribution doesn't *break* during that window, just lags; (b) avoiding empty broadcasts cuts chat noise to zero during downtime.
